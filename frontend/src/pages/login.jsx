...
import { Lock, Mail, Eye, EyeOff, Shield, Users, ClipboardList, TicketCheck, Moon, Sun } from "lucide-react";
...
const Login = () => {
  ...
  return (
    <div className="lmx-login-shell min-h-screen flex flex-col lg:flex-row bg-canvas text-ink relative">
      {/* Theme toggle — top right on mobile, inside hero on desktop */}
      <button
        type="button"
        onClick={toggleTheme}
      ...
      <div className="hidden lg:flex lg:w-1/2 lmx-gradient-hero text-white relative overflow-hidden">
        ...
        <div className="relative flex flex-col justify-center px-10 xl:px-16 py-12 max-w-xl">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h1 className="text-xl font-semibold tracking-tight">Logimax WorkHub</h1>
          </div>
          ...
          <p className="text-base xl:text-lg text-white/80 mb-10 max-w-md leading-relaxed">
            HR operations, project tracking, and ticket management — unified into a single platform.
          </p>
      ...
      {/* Right — login form */}
      <div className="flex-1 flex items-center justify-center px-4 sm:px-8 py-10 lg:py-6">
        <div className="w-full max-w-md tracker-card-plain !border-l-0 p-8 sm:p-10">
          <div className="lg:hidden flex items-center gap-3 mb-8 justify-center">
            <div className="lmx-icon-tile">
              <Shield className="h-5 w-5" />
            </div>
            <h1 className="text-xl font-semibold text-ink">Logimax WorkHub</h1>
          </div>

          <div className="mb-8 text-center lg:text-left">
            <p className="lmx-page-eyebrow mb-2">SIGN IN</p>
            <h3 className="text-2xl sm:text-[28px] font-semibold text-ink tracking-tight">Welcome back</h3>
            <p className="text-sm text-ink-muted mt-2">Sign in to your account to continue</p>
          </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <div
                className={`flex items-center border rounded-tracker-md transition-all bg-surface ${
                  focusedField === "email" ? "border-[var(--tracker-border-focus)] ring-[3px] ring-violet-500/15" : "border-hairline hover:border-ink-subtle"
                }`}
              >
            ...
            <div>
              ...
              <div
                className={`flex items-center border rounded-tracker-md transition-all bg-surface ${
                  focusedField === "password" ? "border-[var(--tracker-border-focus)] ring-[3px] ring-violet-500/15" : "border-hairline hover:border-ink-subtle"
                }`}
              >
          ...
          <p className="text-center text-xs text-ink-subtle mt-8">Internal system — Authorized personnel only</p>
        </div>
      </div>
    </div>
  );
