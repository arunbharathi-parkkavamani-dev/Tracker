...
import {
  ...
  ChevronDown, AtSign, Smile, Bold, Italic, Code, List
} from "lucide-react";
...
const CommentCard = ({ comment, commentReads, participants, currentUserId, onViewFile }) => {
  ...
  const commenterId = comment.createdBy?._id || comment.createdBy;
  const isMe = String(commenterId) === String(currentUserId);

  console.log(comment);
  const firstName = comment.createdBy?.basicInfo?.firstName || "";
  const lastName = comment.createdBy?.basicInfo?.lastName || "";
  const agentName = comment.createdBy?.name || "";
  const isAgent = comment.createdBy?.model === "agents";
  ...
  return (
    <div className={`rounded-xl border transition-all ${!isPublic
      ? "bg-amber-50 border-amber-200 border-dashed"
      : "bg-[var(--tracker-surface)] border-[var(--tracker-border)]"
      }`}>
      {/* Comment Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--tracker-border-soft)]">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[13px] font-semibold text-[var(--tracker-ink)]">{displayName}</span>
              {isAgent && (
              ...
              {isMe && !isAgent && (
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-[var(--module-ticket-light)] text-[var(--module-ticket)]">
                  You
                </span>
              )}
              {!isPublic && (
            ...
            <span className="text-[11px] text-[var(--tracker-ink-subtle)]">
              {formatDate(comment.createdAt || comment.commentedAt)}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {isMe && allRead && (
            <span title="Read by others" className="flex items-center text-blue-500">
              <CheckCheck size={14} />
            </span>
          )}
          {isMe && !allRead && (
            <span className="flex items-center text-[var(--tracker-ink-tertiary)]">
              <Check size={14} />
            </span>
          )}
        </div>
      ...
      <div className="px-4 py-3.5">
        <p className="text-[13.5px] text-[var(--tracker-ink)] leading-relaxed whitespace-pre-wrap break-words">
          {comment.message || comment.comment}
        </p>

        {/* Comment Attachments */}
        {comment.attachments && comment.attachments.length > 0 && (
          <div className="mt-3 pt-3 border-t border-[var(--tracker-border-soft)]">
            <p className="text-[10px] font-semibold text-[var(--tracker-ink-subtle)] uppercase tracking-wide mb-2">
            ...
            <div className="flex flex-wrap gap-2">
              {comment.attachments.map((att, idx) => (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[var(--tracker-border)] bg-[var(--tracker-surface-1)] hover:bg-[var(--tracker-surface-2)] transition-colors text-xs font-medium text-[var(--tracker-ink-muted)] hover:text-[var(--module-ticket)] group max-w-[200px] cursor-pointer text-left"
                >
                  {getFileIcon(att.mimetype)}
                  <span className="truncate">{att.originalName}</span>
                  <span className="text-[10px] opacity-60 shrink-0">{formatBytes(att.size)}</span>
                  <Download size={11} className="opacity-0 group-hover:opacity-100 shrink-0 transition-opacity" />
                </button>
              ))}
            </div>
          </div>
  ...
  const fetchTicketSilently = useCallback(async () => {
    try {
      const populateFields = {
        ...
        "comments.commentedBy": "basicInfo.firstName,basicInfo.lastName,name",
              ...
              <div className={`bg-[var(--tracker-surface)] rounded-2xl border-2 transition-all shadow-xs overflow-hidden ${commentType === "internal"
                ...
                <div className="px-4 pb-3">
                  <textarea
                    ...
                    rows={4}
                    className="w-full text-[13.5px] text-[var(--tracker-ink)] placeholder:text-[var(--tracker-ink-tertiary)] bg-transparent outline-none resize-none leading-relaxed"
                  />
                </div>
