...
class ApprovalEngine {
  ...
  async initializeWorkflow(modelName, document) {
    try {
      ...
      let workflow = null;
      if (deptId) {
        workflow = await models.approvalworkflows.findOne({
          modelName,
          departmentId: deptId,
          isActive: true
        }).lean();
  ...
  async advanceWorkflow(document, approverId, status, comment) {
    try {
      ...
      if (currentIndex + 1 < totalSteps) {
        ...
        const workflow = await models.approvalworkflows.findById(document.workflowId).lean();
        if (!workflow) {
  ...
  /**
   * Helper to resolve approver employee ID from step configuration
   */
  async resolveApprover(step, employeeId, departmentId) {
    switch (step.approverType) {
      ...
      case 'Specific Role': {
        if (!step.specificRoleId) return null;
        const roleEmp = await models.employees.findOne({ 'professionalInfo.role': step.specificRoleId, status: 'Active' }).select('_id').lean();
        return roleEmp?._id;
      }
      case 'Specific User': {
