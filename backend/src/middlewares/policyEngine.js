...
export async function buildQuery({
  ...
  if (!policies || !policies[modelName]) {
    throw new Error(
      `Policy not found for role "${role}" on model "${modelName}"`
    );
  }
  ...
  switch (action.toLowerCase()) {
    ...
    case "create": {
      ...
      if (modelName.toLowerCase() === "attendances") {
        ...
        const isAlternative =
          lastSaturday?.status === "week off" ||
          lastSaturday?.status === "Week off Present";
    ...
    case "update": {
      ...
      if (modelName.toLowerCase() === "attendances") {
        ...
        if (
          !isMale &&
          (!femaleWorkingTime || hours * 60 + minutes < femaleCutOff)
        ) {
          body.status = "Early check-out";
        } else if (
          isMale &&
          (!maleWorkingTime || hours * 60 + minutes < maleCutOff)
        ) {
          body.status = "Early check-out";
        } else {
