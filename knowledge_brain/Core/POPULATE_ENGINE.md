...
5. **Audit Logs**: If data mutated unexpectedly, check the database `AuditLogs` collection to see exactly which user/Trace ID altered the document.
6. **Service Hook Crash**: Check the specific `backend/src/services/<model>.js` to see if a `beforeCreate` or `beforeUpdate` hook threw the exception.
